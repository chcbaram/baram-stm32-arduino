...
STM32H750VBT6. 스케치가 외부 QSPI 플래시에서 그대로 실행됩니다.

[← 패키지 README 로](../README.md)
...
카메라는 `board.cam` 입니다. 보드의 DVP 헤더에 센서를 물리면 QQVGA
160x120 RGB565 를 받아 `board.cam.drawTo(board.lcd)` 로 화면에 올립니다.
`camera.c` 가 **OV2640 과 OV7725 를 런타임에 probe** 하므로 둘 중 무엇을
물려도 동작합니다.

이 보드의 센서에는 **하드웨어 리셋선이 없습니다.** PWDN 이 솔더브리지 SB1 을
통해서만 PA7 에 닿는데 SB1 이 열려 있습니다. 그래서 SCCB 소프트 리셋
(`COM7_SRST`)이 유일한 초기화 경로이고, MCU 만 리셋하는 업로드에서는 센서가
이전 상태를 그대로 갖고 넘어옵니다. **가끔 영상이 안 나오면 USB 를 뺐다
꽂으십시오** — 전원 재인가는 항상 회복시킵니다.
...
**해결됨 (0.1.2):** 디렉터리 순회가 간헐적으로 빈 목록을 돌려주던 문제는
SD 와 무관했습니다. 부트로더가 QUADSPI 메모리맵 모드에서 타임아웃 카운터를 켜둔
것이 원인이었고(ES0392), SD DMA 가 D1 버스를 점유하는 동안 QSPI 명령어 인출이
깨졌습니다. 부트로더 V260830R9 에서 고쳤습니다.
