...
/*
 * 프레임버퍼는 .non_cache(D2 SRAM, 0x30000000) 에 둔다.
 *
 *  - SPI4 TX DMA 가 여기서 직접 읽어간다. D-Cache 를 켠 상태에서 캐시 가능한
 *    메모리를 DMA 소스로 쓰면 CPU 가 쓴 내용이 아직 캐시에만 있어 화면이 깨진다.
 *  - SPI4(APB2)와 DMA1 은 D2 도메인이라 접근도 자연스럽다.
 *
 *  160 x 80 x 2 = 25,600 B.
 */
static uint16_t __attribute__((section(".non_cache"), aligned(64))) frame_buffer[1][HW_LCD_WIDTH * HW_LCD_HEIGHT];
 ...
 * 백라이트.
 *
 * PE10 은 TIM1_CH2N 이라 PWM 으로 밝기 조절이 가능하지만, 부트로더에서는
 * TIM 을 끌어올 이유가 없어 GPIO 로 켜고 끄기만 한다.
 * (LCD_BL 은 active HIGH - chcbaram/stm32h750mv 에서 확인)
 */
...
#ifdef _USE_HW_GPIO
  gpioPinWrite(LCD_BL, value > 0 ? _DEF_HIGH : _DEF_LOW);
#endif
}
...
#if defined(HW_LCD_LVGL) && (HW_LCD_LVGL > 0)
