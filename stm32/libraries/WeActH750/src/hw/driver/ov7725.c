...
#include "hw/driver/ov7725_regs.h"
#include "hw/driver/ov7725.h"
#include "hw/driver/i2c.h"

#ifdef _USE_HW_OV7725

// uint8_t, not the bool the file was ported with: _DEF_I2C1 happens to be 0, so
// bool held it, but a second channel would have collapsed to 1.
static uint8_t i2c_ch = _DEF_I2C1;


// Sensor external clock timer frequency. The board decides it (MCO1 divides
// HSI48 by 4); the exposure and banding arithmetic below reads it back from
// here, so the two cannot drift apart.
#define OMV_XCLK_FREQUENCY      (HW_CAMERA_XCLK_HZ)
...
// The project this file came from carried max()/min() in its own def.h. Arduino
// defines them too, but only for C++, and this is a C file - so they come from
// hw_def.h's cmax/cmin here.
#ifndef max
...
{
  /*
   * The board this was ported from pulses a reset line here. This one has none:
   * the sensor's power down reaches PA7 only through solder bridge SB1, which
   * is open as the board ships, and there is no reset line on the header at
   * all. The sensor comes up on its own and reset() below does it over SCCB
   * instead, by writing COM7_RESET.
   */
